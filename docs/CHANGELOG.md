...
- [新功能] 新增双调度系统（src/trading/signal_scheduler.py）：TradingCalendar 交易日历、PreMarketScheduler 盘前批量信号生成（9:00 ET）、IntradayScheduler 盘中实时信号检查（每分钟轮询）、SignalScheduler 统一调度入口，基于 APScheduler 实现，未安装时自动降级为 no-op 模式
...
- [修复] `CCXTCryptoFetcher` 实时行情方法由 `get_realtime_data` 更名为 `get_realtime_quote` 并返回 `UnifiedRealtimeQuote`，使其与 `DataFetcherManager.get_realtime_quote` 的 `hasattr` 路由约定一致；修复前 CCXT 实时路径从未被管理器调用，所有加密货币实时行情会静默降级到非 CCXT 来源，随之失去 Kraken 直连的质量与 USD notional 一致性。
- [修复] `crypto_context_fetcher.build_crypto_context` 对 CoinGecko 返回的 `market_cap_change_24h_pct`、`btc_dominance`、`ath_change_pct`、`price_change_7d_pct`、`price_change_30d_pct` 等可空字段统一 None 守卫，缺失字段降级为 `N/A` 而不是 `:+.1f` / `:.1f` 抛 TypeError 让整个加密市场上下文从 LLM prompt 中丢失。
- [测试] `tests/test_ccxt_crypto_fetcher.py` 新增 `get_realtime_quote` 路由方法名回归守卫与 `UnifiedRealtimeQuote` 契约断言；新增 `tests/test_crypto_context_fetcher.py`（4 个用例）覆盖 CoinGecko 全球与单币数据 null 字段的降级路径。
...
- [新功能] 大盘复盘支持港股市场：`MARKET_REVIEW_REGION` 新增 `hk` 选项，可生成恒生指数/恒生科技/国企指数等港股大盘复盘报告；`both` 现包含 A股+港股+美股三个市场
